...
/**
 * Item that advances to the next slide in a presentation.
 */
public class NextSlideSwitch extends Item {
    private static final String PRESENTATION_ID_KEY = "PresentationId";

    public NextSlideSwitch(Properties properties) {
        super(properties);
    }
    ...
    public InteractionResult use(Level level, Player player, InteractionHand hand) {
        ...
        if (!tag.contains(PRESENTATION_ID_KEY)) {
            Presentation.LOGGER.warn("Next slide item used without presentation ID");
            return InteractionResult.FAIL;
        }

        String presentationId = tag.getStringOr(PRESENTATION_ID_KEY, "");
        if (presentationId.isEmpty()) {
            Presentation.LOGGER.warn("Next slide item has empty presentation ID");
            return InteractionResult.FAIL;
        }
        ...
        if (manager.nextSlide(presentationId)) {
            ...
            Presentation.LOGGER.info("Presentation '{}': Already at the last slide", presentationId);
        }
