...
/**
 * Item that jumps to the last slide in a presentation.
 */
public class LastSlideSwitch extends Item {
    private static final String PRESENTATION_ID_KEY = "PresentationId";

    public LastSlideSwitch(Properties properties) {
        super(properties);
    }
    ...
    public InteractionResult use(Level level, Player player, InteractionHand hand) {
        ...
        if (!tag.contains(PRESENTATION_ID_KEY)) {
            Presentation.LOGGER.warn("Last slide item used without presentation ID");
            return InteractionResult.FAIL;
        }

        String presentationId = tag.getStringOr(PRESENTATION_ID_KEY, "");
        if (presentationId.isEmpty()) {
            Presentation.LOGGER.warn("Last slide item has empty presentation ID");
            return InteractionResult.FAIL;
        }
        ...
        if (manager.lastSlide(presentationId)) {
            int currentIndex = presentation.getCurrentSlideIndex();
            int totalSlides = presentation.getSlides().size();
            Presentation.LOGGER.info("Presentation '{}': Jumped to last slide ({} / {})", presentationId,
